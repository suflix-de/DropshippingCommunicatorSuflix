...
class ExportController extends Controller
{
    public function sendOrder(Request $request, Response $response, int $orderId): \Symfony\Component\HttpFoundation\Response
    {
        try {
            ...
            // Debug: Dokumente und Varianten analysieren
            $docDebug = [];
            foreach ($documents as $doc) {
                $d = is_array($doc) ? $doc : json_decode(json_encode($doc), true);
                $docDebug[] = [
                    'type'           => $d['type'] ?? 'N/A',
                    'number'         => $d['number'] ?? 'N/A',
                    'numberWithPrefix' => $d['numberWithPrefix'] ?? 'N/A',
                    'hasContent'     => !empty($d['content']),
                    'keys'           => array_keys($d),
                ];
            }

            // Debug: Order item properties
            $itemPropDebug = [];
            foreach (($orderArray['orderItems'] ?? []) as $item) {
                $itemPropDebug[] = [
                    'itemVariationId' => $item['itemVariationId'] ?? '',
                    'properties'      => array_map(fn($p) => ['typeId' => $p['typeId'] ?? '', 'value' => $p['value'] ?? ''], $item['properties'] ?? []),
                ];
                break; // nur erstes Item
            }

            // Debug: Variation alle Felder
            $varDebug = [];
            foreach ($variationExtIds as $vid => $extId) {
                // Neu laden für Debug
                try {
                    $varRepo2 = pluginApp(\Plenty\Modules\Item\Variation\Contracts\VariationRepositoryContract::class);
                    $var2     = $authHelper->processUnguarded(fn() => $varRepo2->findById((int)$vid));
                    if ($var2) {
                        $v2          = json_decode(json_encode($var2), true);
                        $varDebug[$vid] = array_intersect_key($v2, array_flip(['id', 'number', 'externalId', 'model', 'name', 'sku', 'barcode']));
                    }
                } catch (\Throwable $e) {}
            }
